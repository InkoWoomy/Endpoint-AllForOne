...
namespace Endpoint_AllForOne.Services.Mini10
{
    public class Mini10Service : IMini10Service
    {
        public string ResturauntPicker (string input)
        {
            ...
            if (input == "fastfood")
            {
                return "Today, why don't you eat at: " + optionFastFood[rng.Next(optionFastFood.Length)] + "?";
            }
            if (input == "chinese")
            {
                return "Today, why don't you eat at: " + optionChinese[rng.Next(optionChinese.Length)] + "?";
            }
            return "I don't think I can help with that- Try these options: Pizza, Fastfood, Chinese";
